  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white/70 backdrop-blur-lg p-6 rounded-3xl shadow-glass-sm border border-white/50 flex flex-col justify-between h-32 relative overflow-hidden">
           ...
           <p className="text-4xl font-extrabold text-gray-900 z-10">{todayBookings.length}</p>
        </div>
        <div className="bg-white/70 backdrop-blur-lg p-6 rounded-3xl shadow-glass-sm border border-white/50 flex flex-col justify-between h-32 relative overflow-hidden">
           ...
           <p className="text-4xl font-extrabold text-green-600 z-10">{checkedInCount}</p>
        </div>
        <div className="bg-white/70 backdrop-blur-lg p-6 rounded-3xl shadow-glass-sm border border-white/50 flex flex-col justify-between h-32 relative overflow-hidden">
         ...
         <div className="bg-white/80 backdrop-blur-md rounded-3xl shadow-glass border border-white/50 overflow-hidden">
            ...
            <div className="divide-y divide-gray-50">
               {todayBookings.map(booking => {
                   ...
                   return (
                     <div key={booking.id} className="p-4 hover:bg-white/50 transition-colors flex items-center justify-between">
                        <div className="flex items-center space-x-4">
                           <div className="w-12 h-12 rounded-xl bg-indigo-50 flex items-center justify-center font-bold text-indigo-600 shadow-sm">
                             {booking.startTime}
                           </div>
                           <div>
      ...
      {/* PAYMENTS MODULE */}
      {activeTab === 'payments' && (
          <div className="bg-white/80 backdrop-blur-md rounded-3xl shadow-glass border border-white/50 p-6">
              <h3 className="text-xl font-bold text-gray-900 mb-6">Payment Verification</h3>
                       ...
                       {transactions.map(tx => (
                          <tr key={tx.id}>
                             ...
                             <td className="px-6 py-4">
                                <span className={`px-2 py-1 rounded-full text-xs font-bold ${tx.status === 'COMPLETED' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                                   {tx.status}
                                </span>
                             </td>
                             <td className="px-6 py-4">
                                {tx.status === 'PENDING' && (
                                   <button onClick={() => verifyTransaction(tx.id)} className="bg-indigo-600 text-white px-3 py-1 rounded-lg text-xs font-bold">
                                      Verify
                                   </button>
                                )}
                             </td>
      ...
      {/* POS / WALK-IN VIEW */}
      {activeTab === 'walkin' && (
        <div className="bg-white/80 backdrop-blur-md rounded-3xl shadow-glass border border-white/50 p-8">
          <div className="flex items-center space-x-3 mb-8 text-indigo-600">
            <div className="p-3 bg-indigo-100 rounded-xl">
               <UserPlus className="w-6 h-6" />
            </div>
            <h3 className="font-extrabold text-2xl text-gray-900">New Walk-in</h3>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div className="space-y-2">
              <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Select Resource</label>
              <select className="w-full p-4 bg-gray-50 border-transparent rounded-xl font-bold text-gray-800 focus:bg-white focus:ring-2 focus:ring-indigo-500 transition-all">
                {resources.map(r => <option key={r.id} value={r.id}>{r.name} (${r.hourlyRate}/hr)</option>)}
              </select>
            </div>
            {/* ... other inputs ... */}
          </div>
          
          <div className="p-6 bg-gradient-to-r from-gray-900 to-gray-800 rounded-2xl text-white mb-8 shadow-lg">
             ...
             {!showCashierQr ? (
                 <button onClick={() => setShowCashierQr(true)} className="w-full py-3 bg-green-500 hover:bg-green-400 text-white rounded-xl font-bold transition-colors flex justify-center items-center space-x-2">
                 ...
                 <div className="text-center animate-in fade-in zoom-in duration-300">
                     ...
                     <p className="text-yellow-400 font-bold animate-pulse mb-4">Waiting for customer payment...</p>
                     <button onClick={() => { setShowCashierQr(false); alert("Payment Confirmed!"); }} className="bg-white/20 hover:bg-white/30 text-white px-6 py-2 rounded-xl font-bold">
                        Manual Confirm
                     </button>
                 </div>
             )}
      ...
      {activeTab === 'checkin' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="bg-black rounded-3xl p-8 flex flex-col items-center justify-center text-white h-[28rem] relative overflow-hidden shadow-2xl">
          ...
          <div className="bg-white/80 backdrop-blur-md rounded-3xl shadow-glass border border-white/50 flex flex-col h-[28rem] p-6">
             <h3 className="font-bold text-gray-900 mb-4">Simulate GPS Check-in</h3>
             <div className="bg-gray-100 p-4 rounded-xl mb-4">
                 ...
                 <div className="grid grid-cols-2 gap-2">
                     <input type="number" value={gpsSimLat} onChange={e => setGpsSimLat(Number(e.target.value))} className="p-2 rounded border" placeholder="Lat" />
                     <input type="number" value={gpsSimLng} onChange={e => setGpsSimLng(Number(e.target.value))} className="p-2 rounded border" placeholder="Lng" />
                 </div>
                 <div className="mt-2 text-xs text-gray-400">Target Venue: 34.0522, -118.2437</div>
             </div>
             ...
             <div className="flex-1 overflow-y-auto space-y-3 custom-scrollbar">
                {todayBookings.map(b => (
                    <div key={b.id} className="flex justify-between items-center p-3 border rounded-lg">
                        ...
                        <button onClick={() => handleGPSCheckIn(b.id)} className="text-xs bg-indigo-600 text-white px-3 py-1 rounded font-bold">GPS Check-in</button>
                    </div>
                ))}
             </div>
          </div>
